...
"""
                ...
                instruction = """
🔍 **지식자산화 분석 지침** (FAISS DB 기반):
1. **구조화된 분석**: 정보를 논리적으로 분류하고 체계화하세요.
2. **핵심 추출**: 중요한 인사이트와 패턴을 명확히 도출하세요.
3. **실용적 지식**: 재사용 가능한 지식과 베스트 프랙티스를 제시하세요.
4. **연관성 분석**: 정보 간의 연결점과 의존성을 파악하세요.
5. **지식 체계화**: 정보를 카테고리별로 분류하고 태깅하세요.

아래 정보를 바탕으로 체계적이고 구조화된 지식을 생성해주세요:
"""
...
🎯 **주요 역할**:
- 문서 및 데이터 분석을 통한 지식 추출
- 구조화된 정보 정리 및 분류
- 지식베이스 구축 지원
- 베스트 프랙티스 도출

📋 **답변 지침**:
1. **체계적 분석**: 정보를 논리적으로 구조화하여 제시
2. **핵심 요약**: 중요한 포인트를 명확하게 추출
3. **분류 및 태깅**: 정보를 카테고리별로 분류
4. **실행 가능한 인사이트**: 활용 가능한 지식으로 변환

{base_context}

제공된 정보를 바탕으로 체계적이고 구조화된 지식을 만들어드리겠습니다."""
